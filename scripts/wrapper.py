...
data = OPFData(
    ...
    ratio_train,
    ratio_valid,
    param_meta["A_scaling"],
    param_meta["A_threshold"],
    np.float32,
    device=device,
...
if grid_search:
    ...
    param_fit = {
        "max_epochs": 50,
        "module__dimNodeSignals": [4],
        "module__nFilterTaps": [],
        "module__dimLayersMLP": [128, 64, case_info["num_gen"]],
        "module__nSelectedNodes": [],
        "module__poolingSize": [],
    }
    ...
    param_fit = {
        "max_epochs": 50,
        "module__dimLayersMLP": [case_info["num_gen"]],
        "module__dimNodeSignals": [4, 128, 64],
        "module__nFilterTaps": [4, 4],
        "module__nSelectedNodes": [N, N],
        "module__poolingSize": [1, 1],
    }
elif model == "local":
    ...
    param_fit = {
        "max_epochs": 50,
        "module__dimNodeSignals": [4, 128, 64, 1],
        "module__nFilterTaps": [4, 4, 1],
    }
elif model == "localMLP":
    param_meta["local"] = True
    param_fit = {
...
param_fit = {
    "max_epochs": 200,
    "module__dimLayersMLP": [128, 30, 5],
    "module__dimNodeSignals": [29, 64, 32],
    "module__nFilterTaps": [2, 2],
    "module__nSelectedNodes": [30, 30],
    "module__poolingSize": [1, 1],
}
...
G = graphTools.Graph(
    "adjacency", adjacencyMatrix.shape[0], {"adjacencyMatrix": adjacencyMatrix}
)
G.computeGFT()
S, order = graphTools.permDegree(
    G.S / np.max(np.real(G.E))
)  # normalize GSO by dividing by largest e-value
...
if param_meta["local"]:
    ...
    param_grid["module__dimNodeSignals"] = [
        F + [1] for F in param_grid["module__dimNodeSignals"]
    ]
    param_grid["module__nFilterTaps"] = [
        K + [1] for K in param_grid["module__nFilterTaps"]
    ]
    net = skorch.NeuralNetRegressor(
        LocalGNN,
        optimizer=torch.optim.Adam,
        device=device,
        callbacks=[rms_callback],
        module__GSO=S,
        module__index=case_info["gen_index"],
        **param_fit
    )
else:
    param_grid["module__dimLayersMLP"] = [
        M + [case_info["num_gen"]] for M in param_grid["module__dimLayersMLP"]
    ]
    net = skorch.NeuralNetRegressor(
        SelectionGNN,
        optimizer=torch.optim.Adam,
...
if len(param_fit) == 0:
    rms_scorer = sklearn.metrics.make_scorer(
        lambda y, y_pred: data.rms(y, y_pred), False
    )
    l1_scorer = sklearn.metrics.make_scorer(lambda y, y_pred: data.l1(y, y_pred), False)
    linf_scorer = sklearn.metrics.make_scorer(
        lambda y, y_pred: data.linf(y, y_pred), False
    )
    ...
    print(
        "Cost ratio {} | Rate with violation {} | Violation Rate {}".format(
            cost, rate_with_violation, violation_rate
        )
    )
    print(
        "RMS: {} | L1: {} | Linf: {}".format(
            data.rms(yHat, y_test), data.l1(yHat, y_test), data.linf(yHat, y_test)
        )
    )
